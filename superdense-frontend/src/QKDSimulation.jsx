  ...
  const runQKD = async (eveFlag) => {
    ...
    try {
      ...
      if (response.ok) {
        setResults(data);
        if (!data.secure) {
          setError('⚠ QKD key compromised! Eve is present. Generate another key to proceed.');
        }
      } else {
            ...
            >
              ...
              <div className="results-grid">
                ...
                <div className="result-card">
                  ...
                  <p className="qber-status">
                    {results.qber < 0.11 && results.secure ? '✅ Secure' : '⚠ Insecure'}
                  </p>
                </div>
              ...
              {results.alice_bits && results.bob_bits && (
                <div className="comparison-section">
                  <h3>Satellite(ISS) vs GroundStation (First 10 Qubits)</h3>
                  <div className="table-container">
                    <table className="comparison-table">
                      ...
                      <tbody>
                        {results.alice_bits.slice(0, 10).map((aBit, i) => (
                          <tr key={i}>
                            <td>{i + 1}</td>
                            ...
                            <td
                              className={
                                aBit === results.bob_bits[i] ? 'match-yes' : 'match-no'
                              }
                            >
                              {aBit === results.bob_bits[i] ? '✓' : '✗'}
                            </td>
                          </tr>
                        ))}
                      </tbody>
              ...
              <div className="proceed-section">
                {results.secure && results.qber < 0.11 ? (
                  <button className="proceed-button" onClick={handleProceedToSuperdense}>
