...
const cardData = [
  { color: "#060010", title: "QKD", description: "Key distribution summary", label: "Security" },
  { color: "#060010", title: "SDC", description: "Superdense coding flow", label: "Protocol" },
  { color: "#060010", title: "Results", description: "Measurements & stats", label: "Analysis" },
  { color: "#060010", title: "Bloch", description: "State visualization", label: "Viz" },
  { color: "#060010", title: "Eve", description: "Tamper simulation", label: "Adversary" },
  { color: "#060010", title: "Run", description: "Full simulation", label: "Action" },
];
...
const GlobalSpotlight = ({ gridRef, disableAnimations = false, enabled = true, spotlightRadius = DEFAULT_SPOTLIGHT_RADIUS, glowColor = DEFAULT_GLOW_COLOR, }) => {
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e) => {
      ...
      cards.forEach((card) => {
        const cardElement = card; const cardRect = cardElement.getBoundingClientRect();
    ...
    const handleMouseLeave = () => { isInsideSection.current = false; gridRef.current?.querySelectorAll(".card").forEach((card) => { card.style.setProperty("--glow-intensity", "0"); }); if (spotlightRef.current) { gsap.to(spotlightRef.current, { opacity: 0, duration: 0.3, ease: "power2.out" }); } };
...
export default function MagicBento({ textAutoHide = true, enableStars = true, enableSpotlight = true, enableBorderGlow = true, disableAnimations = false, spotlightRadius = DEFAULT_SPOTLIGHT_RADIUS, particleCount = DEFAULT_PARTICLE_COUNT, enableTilt = false, glowColor = DEFAULT_GLOW_COLOR, clickEffect = true, enableMagnetism = true, }) {
      ...
      <BentoCardGrid gridRef={gridRef}>
        <div className="card-responsive grid gap-2">
          {cardData.map((card, index) => {
            ...
            const cardStyle = { backgroundColor: card.color || "#060010", borderColor: "#392e4e", color: "#fff", "--glow-x": "50%", "--glow-y": "50%", "--glow-intensity": "0", "--glow-radius": "200px" };
            return (
